"""
Levantando os próprios erros com raise

raise -> Lança exceções

OBS: O raise não é uma função, é uma palavra resevada assim como def ou qualquer outra em Python

Para simplificar, pense no raise ocmo sendo útil para que possamos criar nossas próprias exceções e mensagens de erro

A forma geral de utilização é:

raise TipoDoErro('Mensagem de erro')

def colore(texto, cor):
    if type(texto) is not str:
        raise TypeError('Texto precisa ser uma string')
    if type(cor) is not str:
        raise TypeError('Cor precisa ser uma string')
    print(f'O texto {texto} será impresso na cor {cor}')


colore('True', 'azul')

# Exemplo refatorado


def colore(texto, cor):
    cores = ('verde', 'amarelo', 'azul', 'branco')
    if type(texto) is not str:
        raise TypeError('Texto precisa ser uma string')
    if type(cor) is not str:
        raise TypeError('Cor precisa ser uma string')
    if cor not in cores:
        raise TypeError(f'A cor {cor} precisar ser uma entre: {cores}')
    print(f'O texto {texto} será impresso na cor {cor}')


colore('True', 'preto')

OBS: O raise assim como o return, finaliza a função, ou seja, nada após o raise é executado.
"""

# Exemplo real


def colore(texto, cor):
    cores = ('verde', 'amarelo', 'azul', 'branco')
    if type(texto) is not str:
        raise TypeError('Texto precisa ser uma string')
    if type(cor) is not str:
        raise TypeError('Cor precisa ser uma string')
    if cor not in cores:
        raise TypeError(f'A cor {cor} precisar ser uma entre: {cores}')
    print(f'O texto {texto} será impresso na cor {cor}')


colore('True', 'preto')
